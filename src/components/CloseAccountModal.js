...
const CloseAccountModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 overflow-y-auto h-full w-full flex items-center justify-center z-[1000]">
      <div className="bg-white p-5 rounded-lg w-7/12 sm:w-2/5">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          ...
          <div className="mb-4 flex flex-col sm:flex-row">
            <span className="block text-gray-400 text-sm font-bold mb-2 sm:mr-4">Want to file UAR</span>
            <label className="inline-flex items-center mb-2 sm:mb-0">
              <input
                ...
                onChange={() => setWantToUAR('yes')}
                className="form-radio"
              />
              <span className="ml-2">Yes</span>
            </label>
            <label className="inline-flex items-center ml-0 sm:ml-4">
              <input
                type="radio"
                name="uar"
                value="no"
                checked={wantToUAR === 'no'}
                onChange={() => setWantToUAR('no')}
                className="form-radio"
              />
            ...
            <label className="flex items-center justify-between">
              <div>
                <input
                  ...
                  onChange={(e) => setChargeClosureFee(e.target.checked)}
              ...
              <div className="flex items-center justify-between mt-2 sm:mt-0">
                <button
                  type="submit"
                  className="bg-gray-200 hover:bg-violet-700 hover:text-white text-gray-400 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                >
                  Close Account
                </button>
              </div>
